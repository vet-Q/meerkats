...
def verify(file_in, target_disease, time_range):
    ...
    # plot(title=f'{target_disease}_altogether', xlabel='timestamp', ylabel='number of outbreaks', x=df.index, y=df.loc[:], save_path=f'{target_disease}_altogether.png')
    # plot(title=f'{target_disease}_sum', xlabel='timestamp', ylabel='number of outbreaks', x=df.index, y=df.loc[:].sum(axis=1), save_path=f'{target_disease}_sum.png')
...
if __name__ == '__main__':    
    ...
    parser.add_argument('--raw_data',default='oie_data.csv', type=str, help='path to OIE csv data')
    parser.add_argument('--preprocessed_data',default='data.csv', type=str, help='path to preprocessed data')    
    parser.add_argument('--target_disease',default='아프리카돼지열병', type=str, help='target disease to train & predict')
    parser.add_argument('--spatial_resolution', default='country',type=str, help='spatial resolution: country or region')
    parser.add_argument('--target_location', default='독일,대한민국',type=str, help='target location to train & predict. Separate by comma. ex) 대한민국,중국,러시아')
