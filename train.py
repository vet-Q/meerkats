...
def train(opt):
    ...
    lr = model_args.lr
    optimizer = torch.optim.AdamW(model.parameters(), lr=lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-3) #옵티마이저로 adamw사용함
    loss_fn = torch.nn.MSELoss()    # loss로 MSE를 사용함
    scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=200, eta_min=0, last_epoch=-1) #schedular를 선언하여, 학습률을 조정할 수 있도록 하는 메서드.
    ...
    for epoch in range(epochs):
        ...
        for bs, (x, y) in enumerate(train_loader):            
            optimizer.zero_grad() #미분에 의한 기울기를 누적하지 않도록 초기화하는 메서드
            x = x.to(device)  # (batch_size, window_size, n_features)  ex) (128, 10, 4)
            y = y.to(device)  # (batch_size, ahead, n_features)  ex) (128, 2, 4)
            y_pred = model(x, x) # 모델에 의한 예측값
            loss = loss_fn(y_pred, y) #loss function에 의한 loss값 계산
            loss.backward()         # back propagation
            train_loss += loss.item() # 무슨 코드인지 잘.
            optimizer.step() #옵티마이저를 작동하게 하는 것인지.
        ...
        with torch.no_grad():
            model.eval() #검증을 실행하고,
            validation_loss = 0 #초기 loss를 0으로 초기화한다
...
if __name__ == '__main__':
    ...
    parser.add_argument('--window_size', default=14,type=int, help='sequence length to make a prediction')  #윈도우 사이즈가 뭔지 잘..
    parser.add_argument('--ahead', type=int, default=14, help='prediction distance')      
